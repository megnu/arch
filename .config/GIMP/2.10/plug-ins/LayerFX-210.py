...
# ---------------------------------------------------------------------
# FOR GIMP 2.10 [according to the sample MareroQ]:
# Replace all old constant names using "GIMP10 Constantize Plug-in" by Trandoduktin (Tim):
# <https://www.gimplearn.net/viewtopic.php/GIMP-Python-Fu-Scripts-Plug-ins/GIMP10-Constantize-Plug-in?f=3&t=1714>
# with a correction layer mode: Hue, Saturation, Color, Value
# and repair Warning: value "0" of type 'gint' is invalid or out of range for property 'area-height' of type 'gint'.
# ---------------------------------------------------------------------
...
class layerfx_base(object):
  mode_list = (LAYER_MODE_NORMAL_LEGACY, LAYER_MODE_DISSOLVE, LAYER_MODE_MULTIPLY_LEGACY, LAYER_MODE_DIVIDE_LEGACY, LAYER_MODE_SCREEN_LEGACY, LAYER_MODE_OVERLAY_LEGACY, LAYER_MODE_DODGE_LEGACY, LAYER_MODE_BURN_LEGACY, LAYER_MODE_HARDLIGHT_LEGACY, LAYER_MODE_SOFTLIGHT_LEGACY, LAYER_MODE_GRAIN_EXTRACT_LEGACY, LAYER_MODE_GRAIN_MERGE_LEGACY, LAYER_MODE_DIFFERENCE_LEGACY, LAYER_MODE_ADDITION_LEGACY, LAYER_MODE_SUBTRACT_LEGACY, LAYER_MODE_DARKEN_ONLY_LEGACY, LAYER_MODE_LIGHTEN_ONLY_LEGACY, LAYER_MODE_HSV_HUE_LEGACY, LAYER_MODE_HSV_SATURATION_LEGACY, LAYER_MODE_HSL_COLOR_LEGACY, LAYER_MODE_HSV_VALUE_LEGACY)
  previewLayer = None
  hiddenLayer = None
  ...
  def apply_noise(self, drawable, srclayer, noise, uselayer):
    noiselayer = gimp.Layer(drawable.image, "%s-noise" % (drawable.name), srclayer.width, srclayer.height, (RGBA_IMAGE, GRAYA_IMAGE)[drawable.image.base_type], 100.0, LAYER_MODE_NORMAL_LEGACY)
    blanklayer = gimp.Layer(drawable.image, "%s-blank" % (drawable.name), srclayer.width, srclayer.height, (RGBA_IMAGE, GRAYA_IMAGE)[drawable.image.base_type], 100.0, LAYER_MODE_NORMAL_LEGACY)
    ...
    pdb.gimp_edit_fill(noiselayer, FILL_FOREGROUND)
    pdb.gimp_edit_fill(blanklayer, FILL_FOREGROUND)
    gimp.set_foreground(255, 255, 255)
    if uselayer:
      srclayer.add_mask(srclayer.create_mask(ADD_MASK_WHITE))
      pdb.gimp_selection_none(drawable.image)
      pdb.gimp_edit_fill(blanklayer, FILL_FOREGROUND)
      ...
      pdb.gimp_selection_none(drawable.image)
      pdb.plug_in_hsv_noise(drawable.image, noiselayer, 1, 0, 0, 255)
      noiselayer.mode = LAYER_MODE_OVERLAY_LEGACY
    noiselayer.opacity = noise
    noiselayer = pdb.gimp_image_merge_down(drawable.image, noiselayer, EXPAND_AS_NECESSARY)
    blanklayer = noiselayer.create_mask(ADD_MASK_COPY)
    noiselayer.add_mask(blanklayer)
    pdb.gimp_selection_none(drawable.image)
    gimp.set_foreground(0, 0, 0)
    pdb.gimp_edit_fill(srclayer.mask, FILL_FOREGROUND)
  ...
  def make_blend_mode_box(self):
    return gimpui.IntComboBox((
      ...
      "Multiply",      LAYER_MODE_MULTIPLY_LEGACY,
      "Divide",        LAYER_MODE_DIVIDE_LEGACY,
      "Screen",        LAYER_MODE_SCREEN_LEGACY,
      "Overlay",       LAYER_MODE_OVERLAY_LEGACY,
      "Dodge",         LAYER_MODE_DODGE_LEGACY,
      "Burn",          LAYER_MODE_BURN_LEGACY,
      "Hard Light",    LAYER_MODE_HARDLIGHT_LEGACY,
      "Soft Light",    LAYER_MODE_SOFTLIGHT_LEGACY,
      "Grain Extract", LAYER_MODE_GRAIN_EXTRACT_LEGACY,
      "Grain Merge",   LAYER_MODE_GRAIN_MERGE_LEGACY,
      "Difference",    LAYER_MODE_DIFFERENCE_LEGACY,
      "Addition",      LAYER_MODE_ADDITION_LEGACY,
      "Subtract",      LAYER_MODE_SUBTRACT_LEGACY,
      "Darken Only",   LAYER_MODE_DARKEN_ONLY_LEGACY,
      "Lighten Only",  LAYER_MODE_LIGHTEN_ONLY_LEGACY,
      #"Hue",           LAYER_MODE_HUE,
      "Hue",           LAYER_MODE_HSV_HUE_LEGACY,
      #"Saturation",    LAYER_MODE_SATURATION,
      "Saturation",    LAYER_MODE_HSV_SATURATION_LEGACY,
      #"Color",         LAYER_MODE_COLOR,
      "Color",         LAYER_MODE_HSL_COLOR_LEGACY,	  
      #"Value",         LAYER_MODE_VALUE
...
class layerfx_drop_shadow(layerfx_base):
  ...
  def showDialog(self):
    ...
    if self.parasitedata:
      ...
      self.mode_box.set_active(LAYER_MODE_MULTIPLY_LEGACY)
  ...
  def makeShadow(self, img, drawable, color, opacity, contour, noise, mode, spread, size, offsetangle, offsetdist, knockout, merge, preview):
    ...
    pdb.gimp_edit_fill(shadowlayer, FILL_FOREGROUND)
    shadowmask = shadowlayer.create_mask(ADD_MASK_BLACK)
    ...
    if (steps > 0):
      ...
      gimp.set_foreground(255, 255, 255)
      pdb.gimp_edit_fill(shadowmask, FILL_FOREGROUND)
    pdb.gimp_selection_none(img)
    if contour > 0:
  ...
  def makeShadow(self, img, drawable, color, opacity, contour, noise, mode, source, choke, size, offsetangle, offsetdist, merge, preview):
    ...
    if merge == 1:
      if source == 1:
        ...
        alphamask = drawable.create_mask(ADD_MASK_ALPHA_MASK_TRANSFER)
...
class layerfx_outer_glow(layerfx_base):
  ...
  def showDialog(self):
	...
	#Warning: value "0" of type 'gint' is invalid or out of range for property 'area-height' of type 'gint'
    self.color_button = gimpui.ColorButton("Glow Color", 80, 1, gimpcolor.RGB(255, 255, 190, 255)) # MrQ
    if self.parasitedata:
      self.color_button.set_color(self.parasitedata["color"])
    elif shelf.has_key(self.shelfkey) == 1:
      self.color_button.set_color(shelf[self.shelfkey]["color"])
    ...
    if self.parasitedata:
      ...
      self.mode_box.set_active(LAYER_MODE_SCREEN_LEGACY)
    self.mode_label.set_mnemonic_widget(self.mode_box)
    self.mode_box.show()
    self.table.attach(self.mode_box, 1, 5, 1, 2)
    self.mode_box.connect("changed", self.preview)
  ...
  def makeGlow(self, img, drawable, color, opacity, contour, noise, mode, spread, size, knockout, merge, preview):
    ...
    if type(color) == gimpcolor.RGB:
      gimp.set_foreground(color)
      pdb.gimp_edit_fill(glowlayer, FILL_FOREGROUND)
      glowmask = glowlayer.create_mask(ADD_MASK_BLACK)
    ...
    if steps > 0:
      ...
      pdb.gimp_selection_grow(img, growamt)
      gimp.set_foreground(255, 255, 255)
      pdb.gimp_edit_fill(glowmask, FILL_FOREGROUND)
    pdb.gimp_selection_none(img)
    if contour > 0:
    ...
    if type(color) != gimpcolor.RGB:
      ...
      glowlayer.add_mask(glowlayer.create_mask(ADD_MASK_BLACK))
      pdb.gimp_selection_all(img)
      gimp.set_foreground(255, 255, 255)
      pdb.gimp_edit_fill(glowlayer.mask, FILL_FOREGROUND)
      glowlayer.remove_mask(MASK_APPLY)
...
class layerfx_bevel_emboss(layerfx_base):
  ...
  def showDialog(self):
    ...
    if self.parasitedata:
      ...
      self.highlightmode_box.set_active(LAYER_MODE_SCREEN_LEGACY)
    ...
    if self.parasitedata:
      ...
      self.shadowmode_box.set_active(LAYER_MODE_MULTIPLY_LEGACY)
  ...
  def makeBevel(self, img, drawable, style, depth, direction, size, soften, angle, altitude, glosscontour, highlightcolor, highlightmode, highlightopacity, shadowcolor, shadowmode, shadowopacity, surfacecontour, use_texture, pattern, scale, tex_depth, invert, merge, preview):
    ...
    bumpmaplayer = gimp.Layer(img, "%s-bumpmap" % (drawable.name), layersize["width"], layersize["height"], imgtype, 100.0, LAYER_MODE_NORMAL_LEGACY)
    ...
    pdb.gimp_edit_fill(highlightlayer, FILL_FOREGROUND)
    gimp.set_foreground(shadowcolor)
    pdb.gimp_edit_fill(shadowlayer, FILL_FOREGROUND)
    gimp.set_foreground(0, 0, 0)
    pdb.gimp_edit_fill(bumpmaplayer, FILL_FOREGROUND)
    highlightmask = highlightlayer.create_mask(ADD_MASK_BLACK)
    shadowmask = shadowlayer.create_mask(ADD_MASK_BLACK)
    ...
    pdb.gimp_selection_none(img)
    if use_texture == 1:
      texturelayer = gimp.Layer(img, "%s-texture" % (drawable.name), int(round(layersize["width"]/(scale/100.0))), int(round(layersize["height"]/(scale/100.0))), imgtype, 100.0, LAYER_MODE_MULTIPLY_LEGACY)
      ...
      texturelayer.fill(FILL_PATTERN)
    ...
    pdb.gimp_edit_fill(highlightmask, FILL_FOREGROUND)
    if surfacecontour > 0:
...
class layerfx_satin(layerfx_base):
  ...
  def makeSatin(self, img, drawable, color, opacity, mode, offsetangle, offsetdist, size, contour, invert, merge, preview):
    ...
    growamt = int(math.ceil(size / 2.0))
    lyrgrowamt = int(round(growamt * 1.2))
    satinlayer = gimp.Layer(img, "%s-satin" % (drawable.name), drawable.width + (lyrgrowamt * 2), drawable.height + (lyrgrowamt * 2), (RGBA_IMAGE, GRAYA_IMAGE)[img.base_type], 100, LAYER_MODE_NORMAL_LEGACY)
    ang = ((offsetangle + 180) * -1) * (math.pi / 180.0)
    offset = (int(round(offsetdist * math.cos(ang))), int(round(offsetdist * math.sin(ang))))
    ...
    pdb.gimp_selection_none(img)
    gimp.set_foreground(0, 0, 0)
    pdb.gimp_edit_fill(satinlayer, FILL_FOREGROUND)
    pdb.gimp_selection_layer_alpha(drawable)
    if drawable.mask != None:
      pdb.gimp_selection_combine(drawable.mask, CHANNEL_OP_INTERSECT)
    alphaSel = pdb.gimp_selection_save(img)
    ...
    blacklayer = gimp.Layer(img, "%s-satinblank" % (drawable.name), satinlayer.width + dx, satinlayer.height + dy, (RGBA_IMAGE, GRAYA_IMAGE)[img.base_type], 100, LAYER_MODE_NORMAL_LEGACY)
    ...
    pdb.gimp_selection_none(img)
    gimp.set_foreground(0, 0, 0)
    pdb.gimp_edit_fill(blacklayer, FILL_FOREGROUND)
    satinmask.mode = LAYER_MODE_DIFFERENCE_LEGACY
    ...
    satinmask = satinlayer.create_mask(ADD_MASK_COPY)
    ...
    if merge == 1:
      ...
      pdb.gimp_selection_invert(img)
      gimp.set_foreground(0, 0, 0)
      pdb.gimp_edit_fill(satinmask, FILL_FOREGROUND)
      satinlayer.remove_mask(MASK_APPLY)
      pdb.gimp_image_set_active_layer(img, drawable)
    gimp.set_foreground(origfgcolor)
    pdb.gimp_selection_load(origselection)
...
class layerfx_stroke(layerfx_base):
  ...
  def showDialog(self):
    ...
    if self.parasitedata:
      ...
      self.mode_box.set_active(LAYER_MODE_NORMAL_LEGACY)
    self.mode_label.set_mnemonic_widget(self.mode_box)
    self.mode_box.show()
  ...
  def makeStroke(self, img, drawable, fill, opacity, mode, size, position, merge, preview):
    ...
    if type(fill) == gimpcolor.RGB:
      ...
      pdb.gimp_edit_fill(strokelayer, FILL_FOREGROUND)
      gimp.set_foreground(origfgcolor)
    elif (type(fill) == tuple or type(fill) == list) and len(fill) == 8:
      origgradient = pdb.gimp_context_get_gradient()
      ...
      pdb.gimp_edit_blend(strokelayer, BLEND_CUSTOM, LAYER_MODE_NORMAL_LEGACY, fill[1], 100.0, 1.0, fill[2], fill[3], False, 1, 0.0, False, measures["start"][0], measures["start"][1], measures["end"][0], measures["end"][1])
      pdb.gimp_context_set_gradient(origgradient)
    elif (type(fill) == tuple or type(fill) == list) and len(fill) == 3:
      ...
      pdb.gimp_edit_fill(strokelayer, FILL_PATTERN)
      pdb.gimp_context_set_pattern(origpattern)
      if fill[1] != 100.0:
...
class layerfx_color_overlay(layerfx_base):
  ...
  def makeOverlay(self, img, drawable, color, opacity, mode, merge, preview):
    ...
    pdb.gimp_selection_none(img)
    gimp.set_foreground(color)
    pdb.gimp_edit_fill(colorlayer, FILL_FOREGROUND)
    if merge == 1:
      origmask = drawable.mask
      layername = drawable.name
      if origmask != None:
      ...
      if drawable.mask != None:
        pdb.gimp_selection_combine(drawable.mask, CHANNEL_OP_INTERSECT)
      alphamask = colorlayer.create_mask(ADD_MASK_SELECTION)
      colorlayer.add_mask(alphamask)
      colorlayer.remove_mask(MASK_APPLY)
      pdb.gimp_image_set_active_layer(img, drawable)
    gimp.set_foreground(origfgcolor)
...
class layerfx_gradient_overlay(layerfx_base):
  ...
  def makeOverlay(self, img, drawable, gradient, gradienttype, repeat, reverse, opacity, mode, centerx, centery, angle, width, merge, preview):
    ...
    if gradienttype >= 6 and gradienttype <= 8:
      pdb.gimp_selection_layer_alpha(drawable)
    pdb.gimp_edit_blend(gradientlayer, BLEND_CUSTOM, LAYER_MODE_NORMAL_LEGACY, gradienttype, 100.0, 1.0, repeat, reverse, False, 1, 0.0, False, measures["start"][0], measures["start"][1], measures["end"][0], measures["end"][1])
    pdb.gimp_selection_none(img)
    if merge == 1:
      origmask = drawable.mask
      layername = drawable.name
      ...
      if drawable.mask != None:
        pdb.gimp_selection_combine(drawable.mask, CHANNEL_OP_INTERSECT)
      alphamask = gradientlayer.create_mask(ADD_MASK_SELECTION)
      gradientlayer.add_mask(alphamask)
      gradientlayer.remove_mask(MASK_APPLY)
      pdb.gimp_image_set_active_layer(img, drawable)
    pdb.gimp_context_set_gradient(origgradient)
...
class layerfx_pattern_overlay(layerfx_base):
  ...
  def makeOverlay(self, img, drawable, pattern, opacity, mode, scale, interpolation_type, merge, preview):
    ...
    pdb.gimp_selection_none(img)
    pdb.gimp_context_set_pattern(pattern)
    patternlayer.fill(FILL_PATTERN)
    if scale != 100.0:
    ...
    if merge == 1:
      ...
      if drawable.mask != None:
        pdb.gimp_selection_combine(drawable.mask, CHANNEL_OP_INTERSECT)
      alphamask = patternlayer.create_mask(ADD_MASK_SELECTION)
      patternlayer.add_mask(alphamask)
      patternlayer.remove_mask(MASK_APPLY)
      pdb.gimp_image_set_active_layer(img, drawable)
    pdb.gimp_context_set_pattern(origpattern)
...
class layerfxplugin(gimpplugin.plugin):
  ...
  def python_layerfx_drop_shadow(
    ...
    contour = 0,
    noise = 0.0,
    mode = LAYER_MODE_MULTIPLY_LEGACY,
    spread = 0.0,
    size = 5,
    offsetangle = 120.0,
    offsetdist = 5.0,
  ...
  def python_layerfx_bevel_emboss(
    ...
    highlightmode = LAYER_MODE_SCREEN_LEGACY,
    highlightopacity = 75.0,
    shadowcolor = gimpcolor.RGB(0, 0, 0, 255),
    shadowmode = LAYER_MODE_MULTIPLY_LEGACY,
    shadowopacity = 75.0,
    surfacecontour = 0,
    use_texture = 0,
    pattern = "",
